...
// and we put the number `50` in to that bucket. Later, we might change what's in
...
// But, what happens when you assign a value that is already held in memory, to a new variable? The JavaScript interpreter has to make a choice. It can do one of
// 2 things:
...
let line2 = line1 
// The string is copied to a new location in memory. The variable `line2` points to the new string. 

line1 = line1 + " Who now? When now?"
// A new string is created. `line1` is adjusted to point at the new string.

console.log('line 1',line1) // "Where now? Who now? When now?"
console.log('line 2',line2); // Has not changed, is still its own copy of "Where now?"
...
// {
//   pizzaCrust: "Deep Dish",
//   calzone: "I am pizza!"
//   

// Special attention should be paid to function calls. When an argument is passed into a function, it is assigned as a parameter in that function's code. Think of it as lining up each argument with each parameter, connected with an assignment operator.
// stuff passed by reference lives on the "heap" as opposed to the value living on the "stack" (variables that disappear when you "return" to the calling function) So the list variable on the call stack is a memory location in the heap
...
//they no nothing about each other after the 
